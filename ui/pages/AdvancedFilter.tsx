...
const AdvancedFilter = () => {
  ...
  const [ filters, setFilters ] = React.useState<AdvancedFilterParams>(() => {
    ...
    return {
      tx_types: getFilterValuesFromQuery(
        ADVANCED_FILTER_TYPES,
        router.query.tx_types,
      ),
      ...
      age_to: age ?
        dayjs().toISOString() :
        getQueryParamString(router.query.age_to),
      age_from: age ?
        dayjs(dayjs().valueOf() - getDurationFromAge(age)).toISOString() :
        getQueryParamString(router.query.age_from),
      token_contract_address_hashes_to_exclude: getValuesArrayFromQuery(
        router.query.token_contract_address_hashes_to_exclude,
      ),
      token_contract_symbols_to_exclude: getValuesArrayFromQuery(
        router.query.token_contract_symbols_to_exclude,
      ),
      token_contract_address_hashes_to_include: getValuesArrayFromQuery(
        router.query.token_contract_address_hashes_to_include,
      ),
      token_contract_symbols_to_include: getValuesArrayFromQuery(
        router.query.token_contract_symbols_to_include,
      ),
      to_address_hashes_to_include: getValuesArrayFromQuery(
        router.query.to_address_hashes_to_include,
      ),
      from_address_hashes_to_include: getValuesArrayFromQuery(
        router.query.from_address_hashes_to_include,
      ),
      to_address_hashes_to_exclude: getValuesArrayFromQuery(
        router.query.to_address_hashes_to_exclude,
      ),
      from_address_hashes_to_exclude: getValuesArrayFromQuery(
        router.query.from_address_hashes_to_exclude,
      ),
    };
  });

  const [ columns, setColumns ] =
    React.useState<Record<ColumnsIds, boolean>>(COLUMNS_CHECKED);
  const {
    data,
    isError,
    isLoading,
  ...
  useApiQuery('tokens', {
    queryParams: { limit: '7', q: '' },
    queryOptions: { refetchOnMount: false },
  });
  useApiQuery('advanced_filter_methods', {
    queryParams: { q: '' },
    queryOptions: { refetchOnMount: false },
  });

  const handleFilterChange = React.useCallback(
    <T extends keyof AdvancedFilterParams>(
      field: T,
      val: AdvancedFilterParams[T],
    ) => {
      setFilters((prevState) => {
        ...
        newState[field] = val;
        onFilterChange(
          newState.age ? omit(newState, [ 'age_from', 'age_to' ]) : newState,
        );
        return newState;
      });
    },
            ...
            <Tr>
              { columnsToShow.map((column) => {
                return (
                  ...
                  >
                    { Boolean(column.name) && (
                      <chakra.span mr={ 2 } lineHeight="24px">
                        { column.name }
                      </chakra.span>
                    ) }
                    <FilterByColumn
  ...
  return (
    <>
      <PageTitle title="Advanced filter" withTextAd/>
      <Flex mb={ 4 } justifyContent="space-between" alignItems="start">
        <Text fontSize="lg" mr={ 3 } lineHeight="24px" w="100px">
          Filtered by:
        </Text>
        { filterTags.length !== 0 && (
          <Link
          ...
          <>
            ...
            >
              <TagLabel>
                ...
                <chakra.span color="text" className="filter-type-span">
                  All
                </chakra.span>
              </TagLabel>
            </Tag>
