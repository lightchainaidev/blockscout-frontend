...
const NavLink = ({ item, onClick, isCollapsed, isDisabled }: Props) => {
    ...
    <Link
      ...
      _hover={{
        [`& *:not(.${ LIGHTNING_LABEL_CLASS_NAME }, .${ LIGHTNING_LABEL_CLASS_NAME } *)`]: {
          color: isDisabled ? 'inherit' : 'primary.100',
        },
      }}
    >
      <Tooltip className="rbt"
        label={ item.text }
        hasArrow={ false }
      ...
      >
        <HStack spacing={ 0 } overflow="hidden">
          ...
          <Text { ...styleProps.textProps } as="span" pl={ 2 }>
            <span className="dropdown-link">{ item.text }</span>
  ...
  return (
    <Box className="lcai-dropdown" as="li" listStyleType="none" w="100%">
      { isInternalLink ? (
